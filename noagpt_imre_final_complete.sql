-- =====================================================
-- NOAGPT IMRE FINAL - SISTEMA COMPLETO E PERFEITO
...
-- 3. INSERIR OS 28 BLOCOS IMRE CANÔNICOS
...
-- 4. CRIAR TABELA DE CONTEXTO DE CONVERSAS IMRE
...
-- 5. CRIAR TABELA DE AVALIAÇÕES INICIAIS (COMPATÍVEL COM CÓDIGO EXISTENTE)
...
-- 6. CRIAR TABELA DE CONVERSAS NOA (COMPATÍVEL COM CÓDIGO EXISTENTE)
...
-- Garantir que a coluna timestamp existe (caso a tabela já exista)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'noa_conversations' AND column_name = 'timestamp') THEN
        ALTER TABLE noa_conversations ADD COLUMN timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'noa_conversations' AND column_name = 'created_at') THEN
        ALTER TABLE noa_conversations ADD COLUMN created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;

-- 7. CRIAR TABELA DE APRENDIZADO IA (COMPATÍVEL COM CÓDIGO EXISTENTE)
...
-- 8. CRIAR TABELA DE PALAVRAS-CHAVE IA (COMPATÍVEL COM CÓDIGO EXISTENTE)
...
-- 9. CRIAR TABELA DE PADRÕES DE CONVERSA IA (COMPATÍVEL COM CÓDIGO EXISTENTE)
...
-- 10. CRIAR TABELA DE PROMPTS (COMPATÍVEL COM NOAGPT BRAIN CENTRAL)
...
-- 11. CRIAR TABELA DE CONTEXTO DE CONVERSAS (COMPATÍVEL COM NOAGPT BRAIN CENTRAL)
...
-- 12. CRIAR TABELA DE RESPOSTAS PADRÃO (COMPATÍVEL COM NOAGPT BRAIN CENTRAL)
...
-- 13. CRIAR TABELA DE PENSAMENTOS FLUTUANTES (COMPATÍVEL COM NOAGPT BRAIN CENTRAL)
...
-- 14. CRIAR FUNÇÃO PARA ATUALIZAR TIMESTAMP
...
-- 15. CRIAR TRIGGERS PARA ATUALIZAR TIMESTAMP
...
DROP TRIGGER IF EXISTS update_conversa_imre_updated_at ON conversa_imre;
...
-- 16. CRIAR FUNÇÃO PARA AVANÇAR BLOCO IMRE
...
-- 17. CRIAR FUNÇÃO PARA GERAR RELATÓRIO IMRE
...
BEGIN
  ...
  FOR campo, valor IN SELECT key, value FROM jsonb_each_text(dados)
  LOOP
    saida := saida || '• ' || COALESCE(
      (SELECT texto FROM blocos_imre WHERE variavel = campo),
      campo
    ) || E'\n';
...
-- 18. CRIAR FUNÇÃO PARA NOAGPT BUSCAR CONTEXTO COMPLETO
...
-- 19. CRIAR FUNÇÃO PARA NOAGPT SALVAR APRENDIZADO INTELIGENTE
...
-- 20. VERIFICAR E DESATIVAR PROMPTS CONFLITANTES
-- Desativa prompts antigos que podem conflitar com o IMRE
UPDATE prompts
SET ativo = false, updated_at = NOW()
WHERE conteudo ILIKE '%o que trouxe%'
   OR conteudo ILIKE '%queixa%'
   OR conteudo ILIKE '%história médica%'
   OR conteudo ILIKE '%alergia%'
   OR conteudo ILIKE '%você sente%'
   OR conteudo ILIKE '%adicionar mais%'
   OR conteudo ILIKE '%hábitos%'
   OR conteudo ILIKE '%validação%'
   OR conteudo ILIKE '%relato espontâneo%'
   OR categoria = 'imre'
   OR especialidade = 'geral';

-- 21. INSERIR PROMPTS INICIAIS
...
🎯 O IMRE é uma técnica de escuta clínica ativa e graduada, voltada à condução cuidadosa do relato do paciente com o mínimo de interferência possível, incentivando que a narrativa surja de forma espontânea, livre e estruturada.
...
-- 22. INSERIR RESPOSTAS PADRÃO INTELIGENTES
...
-- 23. INSERIR PENSAMENTOS FLUTUANTES
...
-- 24. CRIAR ÍNDICES PARA PERFORMANCE
...
CREATE INDEX IF NOT EXISTS idx_noa_conversations_timestamp ON noa_conversations(created_at);
...
-- 26. CRIAR POLÍTICAS RLS
-- Remover políticas existentes primeiro
DROP POLICY IF EXISTS "Allow all for blocos_imre" ON blocos_imre;
DROP POLICY IF EXISTS "Users can manage their own conversa_imre" ON conversa_imre;
DROP POLICY IF EXISTS "Users can manage their own avaliacoes_iniciais" ON avaliacoes_iniciais;
DROP POLICY IF EXISTS "Allow public read access to avaliacoes_iniciais" ON avaliacoes_iniciais;
DROP POLICY IF EXISTS "Allow anonymous evaluation insert" ON avaliacoes_iniciais;
DROP POLICY IF EXISTS "Users can manage their own noa_conversations" ON noa_conversations;
DROP POLICY IF EXISTS "Allow public read access to noa_conversations" ON noa_conversations;
DROP POLICY IF EXISTS "Allow anonymous conversation insert" ON noa_conversations;
DROP POLICY IF EXISTS "Allow all for ai_learning" ON ai_learning;
DROP POLICY IF EXISTS "Allow all for ai_keywords" ON ai_keywords;
DROP POLICY IF EXISTS "Allow all for ai_conversation_patterns" ON ai_conversation_patterns;
DROP POLICY IF EXISTS "Allow all for prompts" ON prompts;
DROP POLICY IF EXISTS "Users can manage their own context" ON conversation_context;
DROP POLICY IF EXISTS "Allow all for default responses" ON default_responses;
DROP POLICY IF EXISTS "Allow all for floating thoughts" ON floating_thoughts;

CREATE POLICY "Allow all for blocos_imre" ON blocos_imre FOR ALL USING (true);
CREATE POLICY "Users can manage their own conversa_imre" ON conversa_imre FOR ALL USING (user_id::uuid = auth.uid() OR user_id IS NULL);
CREATE POLICY "Users can manage their own avaliacoes_iniciais" ON avaliacoes_iniciais FOR ALL USING (user_id::uuid = auth.uid() OR user_id IS NULL);
...
CREATE POLICY "Users can manage their own noa_conversations" ON noa_conversations FOR ALL USING (user_id::uuid = auth.uid() OR user_id IS NULL);
...
CREATE POLICY "Users can manage their own context" ON conversation_context FOR ALL USING (user_id::uuid = auth.uid() OR user_id IS NULL);
...
-- 27. COMENTÁRIOS PARA DOCUMENTAÇÃO
...
-- =====================================================
-- SCRIPT CONCLUÍDO - NOAGPT IMRE FINAL COMPLETO

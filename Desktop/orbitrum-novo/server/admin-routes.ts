...
export function setupAdminRoutes(app: Express) {
  ...
  const requireAdmin = async (req: any, res: any, next: any) => {
    try {
      ...
      console.log('🛠️ MODO TESTE: Permitindo acesso admin temporariamente');
      next();
      return;
  ...
  // ENDPOINT DE TESTE - Verificar dados do dashboard
  app.get('/api/admin/test-dashboard-data', requireAdmin, async (req, res) => {
    try {
      console.log('🔍 TESTE: Verificando dados do dashboard...');
      ...
      const maria = await storage.getUser(2);
      const pedro = await storage.getUser(3);
      const joao = await storage.getUser(4);
      
      const testData = {
        timestamp: new Date().toISOString(),
        storage: {
          totalUsersFromMethod: totalUsers,
        ...
        specificUsers: {
          admin: admin ? {
            ...
            email: admin.email,
            tokens: admin.tokens,
            tokensComprados: admin.tokensComprados
          } : null,
          maria: maria ? {
            id: maria.id,
            email: maria.email,
            tokens: maria.tokens,
            tokensComprados: maria.tokensComprados
          } : null,
          pedro: pedro ? {
            id: pedro.id,
            email: pedro.email,
            tokens: pedro.tokens,
            tokensComprados: pedro.tokensComprados
          } : null,
          joao: joao ? {
            id: joao.id,
            email: joao.email,
            tokens: joao.tokens,
            tokensComprados: joao.tokensComprados
          } : null
        }
      };

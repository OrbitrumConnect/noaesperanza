...
import { orbitalProfessionals, initialUsers } from "./orbital-professionals";
...
export class MemStorage implements IStorage {
  ...
  constructor() {
    ...
    console.log('🌌 SISTEMA ORBITAL - 16 profissionais carregados');
  }
  ...
  // INICIALIZAÇÃO DO SISTEMA ORBITAL - CRÍTICO PARA VISUALIZAÇÃO
  private initializeOrbitalSystem() {
    console.log('🌌 INICIALIZANDO SISTEMA ORBITAL...');
    
    // Limpar profissionais existentes para evitar duplicatas
    this.professionals.clear();
    
    // Carregar todos os 16 profissionais orbitais
    orbitalProfessionals.forEach(professional => {
    ...
    initialUsers.forEach(user => {
      this.users.set(user.id, {
        ...
        tokens: user.tokensComprados || 0,
        adminPermissions: user.adminLevel > 0 ? ['all'] : [],
        isActive: true,
        termsAccepted: true,
        ...
        documentsSubmittedAt: user.createdAt,
        documentsApprovedAt: user.createdAt,
        supabaseId: `initial_${user.id}`,
    ...
    console.log(`✅ ORBITAL: ${this.professionals.size} profissionais carregados`);
    console.log(`✅ USUARIOS: ${this.users.size} usuários iniciais carregados`);
    
    // Atualizar IDs para próximos registros
    this.currentUserId = Math.max(...Array.from(this.users.keys())) + 1;
    this.currentProfessionalId = Math.max(...Array.from(this.professionals.keys())) + 1;
  }

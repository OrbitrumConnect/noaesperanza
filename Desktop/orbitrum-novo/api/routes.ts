...
import { authenticateUser, requireAuth, addDataSourceInfo, type AuthenticatedRequest } from './auth-middleware';
import { dataSystemMonitor } from './data-system-monitor';
  ...
  export function setupHealthRoute(app: Express) {
    app.get('/api/health', async (req: any, res: any) => {
      try {
        ...
        // Verificar status do sistema de dados
        const supabase = getSupabase();
        const dataSystemStatus = supabase ? 'real' : 'demo';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Sistema de dados - Status e informações
  app.get('/api/system/data-status', async (req, res) => {
    try {
      const status = await dataSystemMonitor.checkSystemStatus();
      res.json(status);
    } catch (error) {
  ...
  // Sistema de dados - Estatísticas rápidas
  app.get('/api/system/quick-stats', async (req, res) => {
    try {
      const stats = await dataSystemMonitor.getQuickStats();
      res.json(stats);
    } catch (error) {
  ...
  // Sistema de dados - Forçar sincronização
  app.post('/api/system/force-sync', async (req, res) => {
    try {
      const result = await dataSystemMonitor.forceSync();
      res.json(result);
    } catch (error) {
  ...
  // User dashboard data - Sistema híbrido com dados reais
  app.get("/api/user/dashboard", authenticateUser, addDataSourceInfo, async (req: AuthenticatedRequest, res) => {
    try {
      if (!req.user) {
      ...
      if (req.user.userType === 'professional') {
        dashboardData = await RealDataSystem.getProfessionalDashboardData(req.user.id);
      } else {
        dashboardData = await RealDataSystem.getClientDashboardData(req.user.id);
      }
      
      // Dados de documentos (mock por enquanto)
      ...
      const cashbackEligible = req.user?.plan !== 'free' && documentsStatus.verified;
  ...
  // User statistics - Sistema híbrido com dados reais
  app.get("/api/user/statistics", authenticateUser, addDataSourceInfo, async (req: AuthenticatedRequest, res) => {
    try {
      if (!req.user) {
        return res.status(404).json({ error: "Usuário não encontrado" });
      }
      ...
      const statistics = await RealDataSystem.getUserStatistics(req.user.id);
  ...
  app.post('/api/admin/creditar-tokens', async (req, res) => {
    try {
      ...
      const newTokens = (user.tokensComprados || 0) + parseInt(amount);
      await storage.updateUser(parseInt(userId), {
        tokensComprados: newTokens,
        tokens: (user.tokens || 0) + parseInt(amount) // Atualizar também o total
      });
  ...
  app.post('/api/payment/create-pix', handleAsyncError(async (req, res) => {
    ...
    const pixCode = `00020126330014br.gov.bcb.pix0111${pixKey}520400005303986540${amount.toFixed(2)}5802BR5913PEDRO GALLUF6013RIO DE JANEIRO62${String(7 + transactionId.length).padStart(2, '0')}0503${transactionId}6304F123`;
    
    // QR Code base64 placeholder válido
  ...
  // ================================
  // ENDPOINT DE WITHDRAWALS (SAQUES)
  // ================================
  
  // Listar solicitações de saque
  app.get('/api/admin/withdrawals', async (req, res) => {
    try {
      console.log('🔍 ADMIN: Buscando solicitações de saque...');
      
      // Buscar todas as solicitações de saque
      const withdrawals = await storage.getWithdrawalRequests('all');
      
      console.log(`✅ ADMIN: ${withdrawals.length} solicitações de saque encontradas`);
  ...
  app.post('/api/admin/withdrawals/:id/:action', async (req, res) => {
    try {
      ...
      console.log(`🔧 ADMIN: ${action === 'approve' ? 'Aprovando' : 'Rejeitando'} saque ID ${id}`);
      
      const result = await storage.processWithdrawalRequest(parseInt(id), action as 'approve' | 'reject', 1, reason);
      
      if (result.success) {
        console.log(`✅ ADMIN: Saque ${action === 'approve' ? 'aprovado' : 'rejeitado'} com sucesso`);
        res.json({ success: true, message: `Saque ${action === 'approve' ? 'aprovado' : 'rejeitado'} com sucesso` });
      } else {
        res.status(400).json({ error: result.message || 'Erro ao processar saque' });
      }

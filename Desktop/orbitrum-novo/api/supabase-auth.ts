...
function getBaseUrl(): string {
  ...
  if (process.env.NODE_ENV === 'production' || process.env.REPLIT_DOMAINS) {
    // Usar o domínio Vercel em produção
    return `https://orbitrum-novo.vercel.app`;
  }
...
export async function loginUser(email: string, password: string): Promise<AuthResult> {
  try {
    ...
    // ADMIN MASTER - Login direto com senha específica
    if (email === 'passosmir4@gmail.com' && password === 'm6m7m8M9!horus') {
      console.log('✅ ADMIN MASTER - Login direto autorizado (fallback local)');
      return { success: true, user, message: "Login admin realizado com sucesso" };
    }

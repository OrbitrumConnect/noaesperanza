...
export const useVoiceChat = ({ onTranscription, onAudioResponse }: VoiceChatProps) => {
  ...
  // Rate limiting para evitar spam (invisível ao usuário)
  const { checkRateLimit } = useRateLimit({
    maxRequests: 20, // 20 mensagens por minuto
    windowMs: 60 * 1000,
    cooldownMs: 15 * 1000, // 15 segundos de cooldown
  });

  const startRecording = useCallback(async () => {
    // Verificar rate limit antes de iniciar gravação
    if (!checkRateLimit()) {
      return; // Rate limit atingido, toast já foi mostrado
    }

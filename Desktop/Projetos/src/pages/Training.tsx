...
import { useFreeTrainingLimitProduction } from '@/hooks/useFreeTrainingLimitProduction';
...
const Training = () => {
  ...
  const { 
    ...
    resetTrainingCount,
    loading: trainingLimitLoading
  } = useFreeTrainingLimitProduction('egito-antigo');
  ...
  const nextQuestion = async () => {
    if (currentQuestion < questions.length - 1) {
      ...
      // Para usuários FREE: não ganham pontos/XP/créditos, mas salvam batalha
      if (userType === 'free') {
        console.log('🆓 Usuário FREE: Treino concluído mas sem ganhos (apenas experiência)');
        
        // Salvar batalha no Supabase (sem ganhos)
        await saveBattleResult({
          eraName: 'Egito Antigo',
          questionsTotal: questions.length,
          questionsCorrect: score,
          xpEarned: 0, // FREE não ganha XP
          moneyEarned: 0, // FREE não ganha dinheiro
    ...
    return (
        <div className="min-h-screen bg-background relative overflow-hidden">
          ...
          <div className={`relative z-10 max-w-4xl mx-auto ${isMobile ? 'p-1 h-full overflow-y-auto' : 'p-6'}`}>
            ...
            <div className={`arena-card-digital-started text-center ${isMobile ? 'p-2' : 'p-4'}`}>
              ...
              <div className={`arena-card ${isMobile ? 'p-1.5 mb-2' : 'p-3 mb-3'}`}>
                ...
                <div className={`grid grid-cols-3 gap-1 ${isMobile ? 'text-xs' : 'text-xs'}`}>
                  <div className="text-center">
                    ...
                    <p className={`text-muted-foreground ${isMobile ? 'text-xs' : ''}`}>0,65 créditos</p>
                  </div>
                  <div className="text-center">
            ...
            <div className="arena-card-epic p-8 text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="arena-card p-4">
                  ...
                  <p className="text-2xl font-bold text-battle">+{rewards?.xpEarned || 0}</p>
  ...
  // Tela de jogo principal com perguntas
  return (
    <>
      <div className="min-h-screen bg-background relative overflow-hidden">
      <ParticleBackground />
      
      {/* Fundo Temático Egípcio */}
      <div className="absolute inset-0 z-0">
        <div className="w-full h-full bg-gradient-to-b from-epic/40 to-battle-dark/60" />
        <div className="absolute inset-0 opacity-40" style={{transform: 'scale(1.3)'}}>
      ...
      <div className={`relative z-10 max-w-4xl mx-auto ${isMobile ? 'p-1 h-full overflow-y-auto' : 'p-6'}`}>
        ...
        <div className={`${isMobile ? 'flex justify-between items-center mb-2 px-1' : 'flex items-center justify-between mb-8'}`}>
            <ActionButton 
              ...
              icon={<ArrowLeft />}
              onClick={() => navigate('/app')}
              className={`backdrop-blur-sm bg-battle-dark/80 ${isMobile ? 'text-xs px-0.5 py-0.5 w-1/4' : 'text-xs px-2 py-1'}`}
            >
              {isMobile ? '←' : 'Voltar'}
            </ActionButton>
            
            <div className={`text-center arena-card-epic backdrop-blur-sm bg-card/80 ${isMobile ? 'px-1.5 py-0.5 flex-1 mx-1 w-4/5' : 'px-2 py-1'}`}>
              <h1 className={`font-montserrat font-bold text-epic ${isMobile ? 'text-xs' : 'text-sm'}`}>🏛️ BATALHA</h1>
              <p className={`text-muted-foreground ${isMobile ? 'text-xs leading-none' : ''}`}>{currentQuestion + 1}/{questions.length}</p>
            </div>

            <div className={`text-right arena-card backdrop-blur-sm bg-card/80 ${isMobile ? 'px-1.5 py-0.5' : 'px-1.5 py-1'}`}>
              <p className={`text-muted-foreground ${isMobile ? 'text-xs leading-none' : 'text-sm'}`}>Timer</p>
              <p className={`font-bold text-yellow-400 ${isMobile ? 'text-xs' : 'text-xs'}`}>{timeLeft}s</p>
            </div>
          </div>

          {/* Barra de Progresso Épica */}
          <div className={isMobile ? 'mb-1 mx-1' : 'mb-8'} style={isMobile ? {} : {marginTop: '3%', marginLeft: '8.5%', marginRight: '8.5%'}}>
            <div className={`arena-card backdrop-blur-sm bg-card/80 ${isMobile ? 'p-1 scale-75 w-3/5 mx-auto' : 'p-1.5'}`}>
          ...
          <div className={`relative ${isMobile ? 'mb-1 mx-1' : 'mb-4'}`}>
            <div className={`relative w-full flex items-center justify-between ${isMobile ? 'h-20 mb-1' : 'h-40 mb-6'}`}>
              ...
              <div className={`absolute text-center ${isMobile ? 'left-1 top-5' : 'left-[0%] top-[39%]'}`}>
                <div className={`animate-bounce ${isMobile ? 'mb-0' : 'mb-0.5'} flex justify-center`} style={isMobile ? {} : {animationDuration: '3s'}}>
                  <img 
                    src="/hero-egypt.png" 
                    alt="Herói Egípcio" 
                    className={`${isMobile ? 'w-14 h-14' : 'w-32 h-32'} object-contain`}
                    style={{ 
                ...
                <div className={`arena-card backdrop-blur-sm bg-victory/20 ${isMobile ? 'p-0.5 min-w-12 scale-75' : 'p-1.5 min-w-22'}`}>
              ...
              <div className={`absolute text-center ${isMobile ? 'right-2 top-5' : 'right-[0%] top-[37%]'}`}>
                <div className={`animate-bounce ${isMobile ? 'mb-0' : 'mb-0.5'} flex justify-center`} style={isMobile ? {} : {animationDuration: '3s'}}>
                  <img 
                    src="/enemy-egypt.png" 
                    alt="Inimigo Egípcio" 
                ...
                <div className={`arena-card backdrop-blur-sm bg-destructive/20 ${isMobile ? 'p-0.5 min-w-12 scale-75' : 'p-1.5 min-w-22'}`}>
            ...
            {attackEffect === 'player-attack' && (
              <div className={`absolute pointer-events-none z-[9999] ${isMobile ? 'left-2 top-1/2 transform -translate-y-1/2' : 'left-[-15%] top-[85%]'}`}>
                <div 
                  className={`text-orange-500 ${isMobile ? 'text-lg' : 'text-2xl'}`}
                  style={{
                    animation: 'fireFromPlayerToEnemy 3s ease-out forwards',
                    zIndex: 9999,
                    textShadow: isMobile ? 'none' : '0 0 8px #f97316, 0 0 16px #dc2626, 0 0 24px #fbbf24',
                    filter: isMobile ? 'none' : 'drop-shadow(0 0 6px rgba(249, 115, 22, 0.6)) drop-shadow(0 0 12px rgba(220, 38, 38, 0.5))'
                  }}
                >
                  🔥💥
                </div>
            ...
            {attackEffect === 'enemy-attack' && (
              <div className={`absolute pointer-events-none z-[9999] ${isMobile ? 'right-2 top-1/2 transform -translate-y-1/2' : 'right-[-15%] top-[70%]'}`}>
                <div 
                  className={`text-red-500 ${isMobile ? 'text-lg' : 'text-2xl'}`}
                  style={{
                    animation: 'fireFromEnemyToPlayer 3s ease-out forwards',
                    zIndex: 9999,
                    textShadow: isMobile ? 'none' : '0 0 8px #dc2626, 0 0 16px #b91c1c, 0 0 24px #fbbf24',
                    filter: isMobile ? 'none' : 'drop-shadow(0 0 6px rgba(220, 38, 38, 0.6)) drop-shadow(0 0 12px rgba(185, 28, 28, 0.5))'
                  }}
                >
                  🔥💥
                </div>
          ...
          {/* Pergunta */}
          <div className={`arena-card-epic backdrop-blur-sm bg-epic/10 border border-epic ${isMobile ? 'p-3 mb-4 mx-2 w-full' : 'p-1 mb-2 mt-10 border-2 glow-epic scale-75'}`} style={isMobile ? {} : {marginTop: '11.2%', width: '96%', marginLeft: '2%', marginRight: '2%'}}>
            <div className={`flex items-center justify-center ${isMobile ? 'mb-0.5' : 'mb-1.5'}`}>
            ...
            <h2 className={`font-montserrat font-bold text-center text-foreground ${isMobile ? 'text-xs mb-0.5' : 'text-2xl mb-2.5'}`}>
              {question.question}
            </h2>

            <div className={`grid grid-cols-1 md:grid-cols-2 ${isMobile ? 'gap-1' : 'gap-2'}`}>
            ...
            {showExplanation && (
              <div className="mt-2 p-1 bg-background-soft/80 backdrop-blur-sm rounded-lg border border-card-border">
                <h4 className="font-semibold text-epic mb-0.5 flex items-center text-sm">
                ...
                {question.source && (
                  <div className="mt-0.5 pt-0.5 border-t border-card-border/30">
          ...
          {gamePhase === 'result' && (
            <div className="text-center -mt-8 mx-auto max-w-xs">
              <ActionButton 
                variant="epic" 
                onClick={nextQuestion}
                className="text-xs px-1 py-0.5 backdrop-blur-sm"
              >

...
import { PvPCreditsNotification } from '@/components/notifications/PvPCreditsNotification';
import { usePvPCreditsNotification } from '@/hooks/usePvPCreditsNotification';
import { PvPCreditsDisplay } from '@/components/arena/PvPCreditsDisplay';
...
const Dashboard = () => {
  ...
  const { showNotification, closeNotification, userName } = usePvPCreditsNotification();
  ...
  useEffect(() => {
    const syncUserData = async () => {
      ...
      if (currentUser && currentUser.email !== 'phpg69@gmail.com') {
        ...
        // CPF não existe no profile, usar localStorage
        const userCpf = localStorage.getItem('userCpf') || '';
        if (userCpf) localStorage.setItem('userCpf', userCpf);
        // Phone não existe no profile, usar localStorage
        const userPhone = localStorage.getItem('userPhone') || '';
        if (userPhone) localStorage.setItem('userPhone', userPhone);
        // PIX key não existe no profile, usar localStorage
        const userPixKey = localStorage.getItem('userPixKey') || '';
        if (userPixKey) localStorage.setItem('userPixKey', userPixKey);
        // Birth date não existe no profile, usar localStorage
...
: 'Saque disponível dia 1° do mês • Mínimo 200 créditos (R$ 2,00) • Taxa administrativa 15%'
            ...
            {/* Componente PvP Credits */}
            <PvPCreditsDisplay className="arena-card-epic" />
      ...
      {/* Notificação de Créditos PvP */}
      <PvPCreditsNotification

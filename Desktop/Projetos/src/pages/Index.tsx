...
import { pauseGlobalMusic } from '@/components/ui/persistent-background-music';
import supabase from '@/lib/supabase';

import { calculateWithdrawal } from '@/utils/creditsSystem';
import { calculateWithdrawableAmount } from '@/utils/creditsUnified';
...
// Assets agora estão na pasta public/ - usar caminhos diretos
// Assets agora estão na pasta public/ - usar caminhos diretos
...
const Index = () => {
  ...
  // 🔄 SINCRONIZAR LOCALSTORAGE COM SUPABASE (APENAS USUÁRIOS NÃO-ADMIN)
  useEffect(() => {
    const syncUserData = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      const currentUser = session?.user;
      
      // ✅ SÓ SINCRONIZAR SE NÃO FOR ADMIN
      if (currentUser && currentUser.email !== 'phpg69@gmail.com') {
        if (profile?.email) {
          localStorage.setItem('userEmail', profile.email);
        }
        if (profile?.display_name) {
          localStorage.setItem('userDisplayName', profile.display_name);
        }
        if (profile?.cpf) {
        ...
        if (profile?.pix_key) {
          localStorage.setItem('userPixKey', profile.pix_key);
        }
  ...
  const timelineModules = [
    ...
    },
    {
      ...
      icon: 'globe',
      background: '/digital-landing-bg.jpg', // Usar mesmo background da era digital
  ...
  return (
    <div className={`${isMobile ? 'h-screen overflow-hidden' : 'h-screen overflow-hidden'} relative`}>
          ...
          {isMobile ? (
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-1">
                  <img src="/arena-logo.png" alt="Arena" className="w-5 h-5" />
                  <div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                <img src="/arena-logo.png" alt="Arena do Conhecimento" className="w-10 h-10" />
                <div>
      ...
      <div className={`relative z-10 max-w-6xl mx-auto w-94/100 ${isMobile ? 'p-1 h-full overflow-y-auto' : 'p-6'}`}>
        ...
        {/* Balance & Stats Row */}
        <div className={`${isMobile ? 'space-y-2 mb-3' : 'grid lg:grid-cols-3 gap-8 mb-10'}`}>
          {isMobile ? (
            ...
            <>
              ...
              <CreditsBalanceCard 
                ...
                withdrawAmount={wallet?.balance ? Math.max(0, Math.floor(wallet.balance * 0.8)) : 0}
        ...
        <section className={isMobile ? 'pb-1' : ''}>
          <div className={`grid ${isMobile ? 'grid-cols-4 gap-0.5' : 'grid-cols-1 md:grid-cols-4 gap-3'}`}>
            <div className={`arena-card text-center ${isMobile ? 'p-0.5' : 'p-3'}`}>
              <Users className={`text-battle mx-auto ${isMobile ? 'w-3 h-3' : 'w-7 h-7 mb-2'}`} />
              {!isMobile && <h4 className="font-montserrat font-bold mb-1 text-xs">Duelos Hoje</h4>}
              <p className={`font-bold text-battle ${isMobile ? 'text-xs' : 'text-lg'}`}>
            ...
            <div className={`arena-card text-center ${isMobile ? 'p-0.5' : 'p-3'}`}>
              <Target className={`text-victory mx-auto ${isMobile ? 'w-3 h-3' : 'w-7 h-7 mb-2'}`} />
              {!isMobile && <h4 className="font-montserrat font-bold mb-1 text-xs">Taxa Vitória</h4>}
              <p className={`font-bold text-victory ${isMobile ? 'text-xs' : 'text-lg'}`}>{winRate}%</p>
            ...
            <div className={`arena-card text-center cursor-pointer hover-scale ${isMobile ? 'p-0.5' : 'p-3'}`} onClick={() => navigate('/knowledge')}>
              <Database className={`text-primary-glow mx-auto ${isMobile ? 'w-3 h-3' : 'w-7 h-7 mb-2'}`} />
              {!isMobile && <h4 className="font-montserrat font-bold mb-1 text-xs">Conhecimento</h4>}
              {isMobile ? (
                <p className="text-xs text-muted-foreground">Acervo</p>
              ) : (
                <>
                  <p className="text-primary-glow text-xs mb-2">Ver itens</p>
                  <ActionButton variant="epic" className="w-full text-xs py-1.5">
                    Explorar
                  </ActionButton>
                </>
              )}
            ...
            <div className={`arena-card text-center ${isMobile ? 'p-0.5' : 'p-3'}`}>
              <UserPlus className={`text-epic mx-auto ${isMobile ? 'w-3 h-3' : 'w-7 h-7 mb-2'}`} />
              {!isMobile && <h4 className="font-montserrat font-bold mb-1 text-xs">Indicar Amigo</h4>}
              {isMobile ? (

  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // 🔄 SINCRONIZAR DADOS DO USUÁRIO LOGADO (APENAS SE NÃO FOR ADMIN)
        const { data: { user } } = await supabase.auth.getUser();
        if (user?.email && user.email !== 'phpg69@gmail.com') {
        ...
        // 🔄 SINCRONIZAR DADOS DO USUÁRIO CADASTRADO (APENAS SE NÃO FOR ADMIN)
        const { data: { user } } = await supabase.auth.getUser();
        if (user?.email && user.email !== 'phpg69@gmail.com') {
          localStorage.setItem('userEmail', user.email);
        }
        if (displayName && user?.email !== 'phpg69@gmail.com') {
          localStorage.setItem('userDisplayName', displayName);
        }
        if (cpf && user?.email !== 'phpg69@gmail.com') {
          localStorage.setItem('userCpf', cpf);
        }
        if (phone && user?.email !== 'phpg69@gmail.com') {
          localStorage.setItem('userPhone', phone);
        }
        if (pixKey && user?.email !== 'phpg69@gmail.com') {
          localStorage.setItem('userPixKey', pixKey);
        }
        if (birthDate && user?.email !== 'phpg69@gmail.com') {
        ...
        try {
          if (user) {
            await supabase.from('profiles').insert({
              user_id: user.id,
              display_name: displayName, // ✅ Salvar nome no perfil

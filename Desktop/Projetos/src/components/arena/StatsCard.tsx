...
export const StatsCard = ({ title, value, type, percentage, isImproving = true }: StatsCardProps) => {
  ...
  return (
    <div className="arena-card p-5 hover-scale">
      <div className="flex items-center justify-between mb-4">
        <div className={`${getColor()}`}>
          {getIcon()}
        </div>
        <div className={`text-sm ${isImproving ? 'text-victory' : 'text-destructive'}`}>
          {isImproving ? '+' : '-'}{percentage}%
        </div>
      </div>
      
      <h4 className="font-montserrat font-semibold text-base mb-2">{title}</h4>

...
export const CreditsBalanceCard = ({ 
  ...
  return (
            <div className={`arena-card-epic ${isMobile ? 'p-2 scale-90 max-h-[200px] overflow-hidden w-9/10 mx-auto' : 'p-4'}`}>
      ...
      {earnedCredits > 0 && (
        <div className="mb-2 p-2 bg-victory/10 rounded-lg border border-victory/20">
          <div className="flex items-center space-x-2">
            <TrendingUp className="w-3 h-3 text-victory" />
            <span className="text-xs text-victory font-semibold">
      ...
      <div className="border-t border-muted/20 pt-2">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-xs font-bold">{Math.round(withdrawAmount)} créditos</span>
        </div>
        
        {canWithdraw ? (
          <Button className="btn-epic w-full text-xs py-1">
            Solicitar Devolução
          </Button>
        ) : (
          <div className="text-center">
